...
namespace Calcpad.Core
{
    ...
    {
        // Static flag to control unit stripping - set by MathParser
        internal static bool UnitsOff { get; set; } = false;
        ...
        {
            IValue assignValue = value;

            // Strip units if UnitsOff mode is active
            if (UnitsOff)
            {
                if (value is RealValue real && real.Units != null)
                    assignValue = new RealValue(real.D);
                else if (value is ComplexValue complex && complex.Units != null)
                    assignValue = new ComplexValue(complex.Complex);
            }

            _value = assignValue;
            _isIntialised = true;
            OnChange?.Invoke();
            if (assignValue is Vector vector)
                vector.OnChange += Change;
            else if (assignValue is Matrix matrix)
