...
namespace Calcpad.Core
{
    public partial class ExpressionParser
    {
        ...
        KeywordResult ParseKeyword(ReadOnlySpan<char> s, ref Keyword keyword)
        {
            ...
            switch (keyword)
            {
                ...
                case Keyword.End_Svg:
                    ParseKeywordEndSvg();
                    break;
                case Keyword.UnitsOff:
                    if (_calculate && _condition.IsSatisfied)
                        _parser.UnitsOff = true;
                    else if (_isVisible)
                        _sb.Append($"<p{HtmlId} class=\"cond\">#unitsoff</p>");
                    break;
                case Keyword.UnitsOn:
                    if (_calculate && _condition.IsSatisfied)
                        _parser.UnitsOff = false;
                    else if (_isVisible)
                        _sb.Append($"<p{HtmlId} class=\"cond\">#unitson</p>");
                    break;
        ...
        private void ParseKeywordSvg(ReadOnlySpan<char> s)
        {
            if (_calculate && _condition.IsSatisfied)
            {
                // Parse dimensions from #svg{width; height}
                ...
                if (startIdx != -1 && endIdx > startIdx)
                {
                    ...
                    if (parts.Length >= 1 && double.TryParse(parts[0].Trim(), out var w))
                        _svgWidth = w;
                    if (parts.Length >= 2 && double.TryParse(parts[1].Trim(), out var h))
                        _svgHeight = h;
                }

                _inSvgBlock = true;
                _svgElements = new StringBuilder();
            ...
            {
                _sb.Append($"<p{HtmlId} class=\"cond\">#svg</p>");
            }
        }

        private void ParseKeywordEndSvg()
        {
            if (_calculate && _condition.IsSatisfied && _inSvgBlock)
            {
                // Generate complete SVG
                var svg = new StringBuilder();
                svg.AppendFormat(System.Globalization.CultureInfo.InvariantCulture,
                    "<svg width=\"{0:F0}\" height=\"{1:F0}\" viewBox=\"0 0 {0:F0} {1:F0}\" xmlns=\"http://www.w3.org/2000/svg\" style=\"font-family:Segoe UI;font-size:10px;\">\n",
                    _svgWidth, _svgHeight);
                svg.Append(_svgElements);
                svg.AppendLine("</svg>");

                _sb.Append(InsertAttribute(svg.ToString(), HtmlId));
                _inSvgBlock = false;
                _svgElements = null;
            }
            else if (_isVisible)
            {
                _sb.Append($"<p{HtmlId} class=\"cond\">#end svg</p>");
            }
        }
    }
}
