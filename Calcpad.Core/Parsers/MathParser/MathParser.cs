...
namespace Calcpad.Core
{
    public partial class MathParser
    {
        ...
        internal bool UnitsOff
        {
            get => Variable.UnitsOff;
            set => Variable.UnitsOff = value;
        }
        ...
        // Public method to get variable value as double (for PyCalcpad)
        public double GetVariableReal(string name) => GetVariable(name).Re;
        ...
        /// <summary>
        /// Sets a variable to any IValue (scalar, vector, or matrix)
        /// Used by TriangleMeshParser to store mesh results
        /// </summary>
        internal void SetVariable(string name, in IValue value)
        {
            if (_variables.TryGetValue(name, out var variable))
        ...
        public string[][] GetMatrix(string name, char type)
        {
            if (!_variables.TryGetValue(name, out var variable))
                throw Exceptions.VariableNotExist(name);
        ...
        public void SetMatrix(ReadOnlySpan<char> name, string[][] data, char type, bool isHp)
        {
        ...
        public string ResultAsVal
        {
            get
            {
                ...
                // Use HTML visual format for vectors and matrices
                ...
                // Fallback to old format if not vector/matrix
                var sb = new StringBuilder();
                sb.Append('[');
                sb.Append(']');
                return sb.ToString();
