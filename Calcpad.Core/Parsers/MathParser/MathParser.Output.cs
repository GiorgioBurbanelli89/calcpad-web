        ...
        private sealed class Output
        {
            ...
            internal string Render(OutputWriter.OutputFormat format)
            {
                ...
                else
                {
                    ...
                    if (_parser._isCalculated)
                    {
                        ...
                        // For matrices/vectors, compare normalized content to avoid duplication
                        var shouldAddResult = hasOperators && !AreMatrixContentsEqual(res, subst) || string.IsNullOrEmpty(subst);
                        if (shouldAddResult)
                        {
            ...
            private string RenderRpn(Token[] rpn, bool substitute, OutputWriter writer, out bool hasOperators)
            {
                    ...
                    else
                    {
                        ...
                        if (st == NegateString)
                            ...
                            else if (tt == TokenTypes.MatrixIndex)
                            {
                                ...
                                t.Content = RenderParameters(t, b, t.ParameterCount, false, true);
                ...
                void RenderNegationToken(RenderToken t, RenderToken b, ref bool hasOperators)
                {
                    ...
                    bool isSimpleTerm = b.Type == TokenTypes.Variable || b.Type == TokenTypes.Constant;
                    ...
                    // When negating a simple term (variable or constant), the result is still
                    // a single term (-a, -b, -10), not an expression with operators.
                    // Set Order to DefaultOrder so vector/matrix rendering doesn't add parentheses.
                    if (isSimpleTerm)
                        t.Order = Token.DefaultOrder;
                ...
                void RenderMatrixToken(RenderToken t, RenderToken b)
                {
                    if (writer is HtmlWriter)
                    {
                        // Collect rows from stack - each row was grouped by RowDivisor
                        var rows = new List<string>();
                        rows.Add(b.Content);
                        for (int j = 0; j < t.ParameterCount; ++j)
                        {
                            var a = stackBuffer.Pop();
                            rows.Insert(0, a.Content);
                            if (a.Level > t.Level)
                                t.Level = a.Level;
                        ...
                        // Generate visual matrix HTML
                        var sb = new StringBuilder();
                        sb.AppendLine("<span class=\"matrix\">");
                        foreach (var row in rows)
                        {
                            sb.Append("<span class=\"tr\"><span class=\"td\"></span>");
                            var cells = SplitByDelimiter(row, CellSep);
                            foreach (var cell in cells)
                            {
                                sb.Append($"<span class=\"td\">{cell.Trim()}</span>");
                            }
                ...
                void RenderVectorToken(RenderToken t, RenderToken b)
                {
                    if (writer is HtmlWriter)
                    {
                        // Collect elements from stack with their Order info
                        var elements = new List<(string Content, sbyte Order)>();
                        elements.Add((b.Content, b.Order));
                        t.Level = b.Level;
                        ...
                        {
                            var a = stackBuffer.Pop();
                            elements.Insert(0, (a.Content, a.Order));
                            if (a.Level > t.Level)
                                t.Level = a.Level;
                            t.MinOffset = Math.Min(a.MinOffset, t.MinOffset);
                            t.MaxOffset = Math.Max(a.MaxOffset, t.MaxOffset);
                        ...
                        for (int i = 0; i < elements.Count; i++)
                        {
                            if (i > 0)
                                sb.Append(" ");
                            // Wrap expressions with operators in parentheses for clarity
                            var elem = elements[i];
                            if (NeedsParenthesesInVector(elem.Content, elem.Order))
                                sb.Append($"&ensp;({elem.Content})&ensp;"); // Wide spacing around expressions
                            else
                                sb.Append(elem.Content);
                        }
                        sb.Append("<b class=\"b0\">]</b>");
                        t.Content = sb.ToString();
                    }
                ...
                // Check if an element needs parentheses when displayed in a vector/matrix
                // This is true for expressions with operators (like a+b, x*y, etc.)
                bool NeedsParenthesesInVector(string content, sbyte order)
                {
                    // If order > default, it's an expression with operators
                    if (order > Token.DefaultOrder)
                        return true;

                    // Also check for operator symbols outside HTML tags
                    bool inTag = false;
                ...
                // Helper to split a string by a delimiter, skipping HTML entities (&emsp; &nbsp; etc.)
                List<string> SplitByDelimiter(string input, string delimiter)
                {
                    ...
                    int start = 0;
                    int pos = 0;
                    while ((pos = input.IndexOf(delimiter, start, StringComparison.Ordinal)) >= 0)
                    {
                        if (delimiter[0] == ';')
                        {
                            bool isEntity = false;
                            for (int k = pos - 1; k >= start && k >= pos - 10; k--)
                            {
                                char ch = input[k];
                                if (ch == '&') { isEntity = true; break; }
                                if (!char.IsLetterOrDigit(ch) && ch != '#') break;
                            }
                            if (isEntity)
                            {
                                start = pos + 1;
                ...
                string RenderParameters(RenderToken t, RenderToken b, int count, bool matrix = false, bool wrapExpressionsInParens = false)
                {
                    // For matrix cells in HTML, wrap expressions with operators in parentheses
                    var bContent = b.Content;
                    if (wrapExpressionsInParens && writer is HtmlWriter && NeedsParenthesesInVector(b.Content, b.Order))
                        bContent = $"&ensp;({b.Content})&ensp;"; // Wide spacing around expressions

                    var s = bContent;
                    t.Level = b.Level;
                    t.MinOffset = b.MinOffset;
                    t.MaxOffset = b.MaxOffset;
                    var d = matrix ? writer.FormatOperator('|') :
                            (wrapExpressionsInParens && writer is HtmlWriter) ? CellSep : div;
                    for (int j = 0; j < count; ++j)
                    {
                        var a = stackBuffer.Pop();
                        var aContent = a.Content;
                        if (wrapExpressionsInParens && writer is HtmlWriter && NeedsParenthesesInVector(a.Content, a.Order))
                            aContent = $"&ensp;({a.Content})&ensp;"; // Wide spacing around expressions

                        s = string.Concat(aContent, d, s);
                        if (a.Level > t.Level)
                            t.Level = a.Level;
            ...
            // Compare two matrix/vector HTML strings to check if they have the same content
            // This avoids duplicating the result when substitution produces the same output
            private static bool AreMatrixContentsEqual(string res, string subst)
            {
                if (string.IsNullOrEmpty(res) || string.IsNullOrEmpty(subst))
                    return false;

                // If both contain matrix class, extract and compare cell contents
                if (res.Contains("class=\"matrix\"") && subst.Contains("class=\"matrix\"") ||
                    res.Contains("class=\"b0\"") && subst.Contains("class=\"b0\""))
                {
                    // Extract numeric content by removing HTML tags and comparing
                    var resContent = ExtractNumericContent(res);
                    var substContent = ExtractNumericContent(subst);
                    return string.Equals(resContent, substContent, StringComparison.Ordinal);
                }

                return string.Equals(res, subst, StringComparison.Ordinal);
            }

            // Extract numeric/text content from HTML, removing tags
            private static string ExtractNumericContent(string html)
            {
                var sb = new StringBuilder();
                bool inTag = false;
