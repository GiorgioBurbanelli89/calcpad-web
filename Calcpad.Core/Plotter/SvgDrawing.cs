...
namespace Calcpad.Core
{
    internal class SvgDrawing
    {
        ...
        private void AddStyle(int n)
        {
            ...
            .AppendLine($".plot .Grid, .plot .Frame, .plot .Axis, .plot .Contour{{fill:none; stroke-width:{Convert(ScaleFactor)};}}")
            .AppendLine(".plot .Grid {stroke:rgba(0, 0, 0, 0.08);}")
            .AppendLine(".plot .Frame {stroke:rgba(0, 0, 0, 0.24);}")
            .AppendLine(".plot .Axis {stroke: Black;}")
            .AppendLine($".plot .Contour {{stroke: rgba(50, 50, 50, 0.5); stroke-width:{Convert(ScaleFactor * 0.8)};}}")
